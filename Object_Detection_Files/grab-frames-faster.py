...
def process_frame(frame):
    img = frame
    n_objects = 1
    result, objectInfo = getObjects(n_objects, img, thresh, nms,  frame_height=frame_height, frame_width=frame_width, draw=False)
    # print(objectInfo)
    ## Check iff one objected is detected. If number of objects detected != 1 then continue in the loop
    if len(objectInfo) != 3:
      return img
    object_center = objectInfo[-1]
    object_area = objectInfo[-2]/1000
    # print(object_center)
    ...
    if v_mod < 0:
        print(f"direction is left to right, magnitude is {v_mod}, area of object = {object_area}")
    if v_mod > 0:
        print(f"direction is right to left, magnitude is {v_mod}, area of object = {object_area}")
